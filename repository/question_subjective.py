from config.db import connect_db
from typing import Dict, Any
from datetime import date 

@connect_db
def insert_question_subjective(conn,qid:int,item_id:int,correct_answer:str ):
    try:
        cur = conn.cursor()
        sql = 'INSERT INTO question_subjective (qid, item_id, correct_answer) VALUES (%s, %s, %s)'
        values = (qid, item_id, correct_answer)
        cur.execute(sql, values)
        cur.close()
        return True
    except Exception as e:
        cur.close()
        print(f"Error inserting question subjective: {e}")
    return False

@connect_db
def update_question_subjective(conn,id:int,details:Dict[str,Any]):
    try:
        cur = conn.cursor()
        parmas = ['{}=%s'.format(k) for k in details.keys()]
        values = tuple(details.values())
        sql = 'UPDATE question_subjective SET {} WHERE id={}'.format(', '.join(parmas), id)
        cur.execute(sql, values)
        cur.close()
        return True
    except Exception as e:
        cur.close()
        print(f"Error updating question subjective: {e}")
    return False

@connect_db
def delete_question_subjective(conn,id:int):
    try:
        cur = conn.cursor()
        sql = 'DELETE FROM question_subjective WHERE id=%s'
        values = (id,)
        cur.execute(sql, values)
        cur.close()
        return True
    except Exception as e:
        cur.close()
        print(f"Error deleting question subjective: {e}")
    return False

@connect_db
def select_all_question_subjective(conn):
    try:
        cur = conn.cursor()
        sql = 'SELECT * FROM question_subjective'
        cur.execute(sql)
        results = cur.fetchall()
        cur.close()
        return results
    except Exception as e:
        cur.close()
        print(f"Error selecting all question subjective: {e}")
    return None

@connect_db
def select_single_question_subjective(conn,id:int):
    try:
        cur = conn.cursor()
        sql = 'SELECT * FROM question_subjective WHERE id=%s'
        values = (id,)
        cur.execute(sql, values)
        result = cur.fetchone()
        cur.close()
        return result
    except Exception as e:
        cur.close()
        print(f"Error selecting single question subjective: {e}")
    return None